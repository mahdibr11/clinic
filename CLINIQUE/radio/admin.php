...
if (isset($_POST['password'])) {
    ...
    if ($enteredPassword === $password) {
        ...
        $sqlFuture = "SELECT * FROM reservationRadiologie WHERE CONCAT(date, ' ', time) > '$currentDateTime'";
        $resultFuture = $conn->query($sqlFuture);

        // Query the database for reservations where date and time are less than or equal to now
        $sqlPast = "SELECT * FROM reservationRadiologie WHERE CONCAT(date, ' ', time) <= '$currentDateTime'";
        $resultPast = $conn->query($sqlPast);
    } else {
        // Password is incorrect, display an error message
        echo "Incorrect password. Access denied.";
        // You may choose to exit here or redirect the user to another page
        exit();
    }
} else {
    // Password form not submitted, display the password form
    echo '<form method="post" action="">
              Password: <input type="password" name="password">
              <input type="submit" value="Submit">
          </form>';
    // You may choose to exit here or continue without displaying the page content
    exit();
}
