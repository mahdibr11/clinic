...
function checkAndReserveSlot($time, $date, $nom, $prenom, $telephone, $email) {
    ...
    $sql = "SELECT * FROM 	reservationmaternité WHERE date = '$date' AND time = '$time'";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $sql = "INSERT INTO reservationmaternité (date, time, nom, prenom, telephone, email)
...
if (isset($_POST["date"])) {
    $date = $_POST["date"];
    $sql = "SELECT time FROM reservationmaternité WHERE date = '$date'";
    $result = $conn->query($sql);
