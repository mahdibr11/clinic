...
$sqlFuture = "SELECT * FROM reservationdialyse WHERE CONCAT(date, ' ', time) > '$currentDateTime'";
$resultFuture = $conn->query($sqlFuture);

// Query the database for reservations where date and time are less than or equal to now
$sqlPast = "SELECT * FROM reservationdialyse WHERE CONCAT(date, ' ', time) <= '$currentDateTime'";
